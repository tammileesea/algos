...
public class BinarySearchSolution {
    ...
    public int Search_2(int[] nums, int target)
    {
        ...
        while (lo <= hi)
        {
            ...
            if (nums[mid] > target)
                hi = mid - 1; //since the target is smaller than the value at mid, high becomes the index right below mid, thus adjusting the range to below the current mid
            else
                lo = mid + 1; //same idea here but adjusting low to be right above mid
        }
...
// ---------- NOTES/THOUGHTS -----------
// in the other solution, they update the high or low vals depending on whether or not the target is smaller or larger
// than the current val we're looking at

// I set mid and used it as the halfway point then
// I change low and high simultaneously



// V GOOD EXPLANATION!!!!!
// Binary search is often a topic that's easy to be explained on the abstract level, 
// but when it comes to writing bug free implementations, it's rather difficult.
...
// the worst case scenario is we need to go through every single element (O(n)). 
// However, when these elements are sorted, we are able to take the privilege 
// of this extra information to bring down the search time to O(log n), 
...
// They will store array indexes and they work like a boundary such 
// that we will only be looking at elements inside the boundary.
...
// It separates our boundary into 2 parts. Remember how I said binary search works by keep cutting the elements in half, 
// the mid element works like a traffic police, it indicates us which side do we want to cut our boundary to.

// Note when an array has even number of elements, 
...
// For example, If our target is greater than mid, this means it must exist in the right of mid . 
// In this case, there is no reason to even keep a record of all the numbers to its left. 
// And this is the fundamental mechanics of binary search - keep shrinking the boundary.
...
// while (lo < hi) { ... }
// The while loop only exits when lo == hi, which means there's only one element left. 
// And if we implemented everything correctly, that only element should be our answer(assume if the target is in the array).
...
// Ill demonstrate a few ways of calculating mid from the worst to the best.
...
// and as Ill be explaining in a later section, a bad choice will lead to an infinity loop.
...
// and we can exclude it very confidently using hi = mid - 1. Otherwise, mid still has the potential to be the target, 
...
// while(lo < hi) { ... }
// Why? Because this way, the only condition the loop exits is lo == hi. I know they will be pointing to the same element, 
// and I know that element always exists.

// 5. Avoid infinity loop
// Remember I said a bad choice of left or right mid will lead to an infinity loop? Lets tackle this down.
// Example:
...
// If the logic fell into the else statement, since we are using the left/lower mid, its simply not doing anything. 
// It just keeps shrinking itself to itself, and the program got stuck.
// We have to keep in mind that, the choice of mid and our shrinking logic has to work together in a way that every time, 
...
// Did the boundary shrink correctly?
...
// Because this problem is a failrly easy, the implementions may be pretty straight forward and 
// you may wonder why do I need so many rules. However, binary search problems can get much much more complex, 
// and without consistent rules, it's very hard to write predictable code. 
